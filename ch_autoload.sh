...
CH_CONTAINER="hdx-oss-ch-server-1"
DB="ml_data_mart"
...
ch() { docker exec -i "$CH_CONTAINER" clickhouse-client "${CLIENT_FLAGS[@]}" -q "$1"; }

sanitize_table() {
  echo "$1" | sed 's/[^a-zA-Z0-9]/_/g;s/_\{1,\}$//' | tr '[:upper:]' '[:lower:]'
}

in_container_file_exists(){ docker exec -i "$CH_CONTAINER" bash -lc "test -f \"$1\""; }
host_md5(){ md5sum "$1" | awk '{print $1}'; }
in_container_md5(){ docker exec -i "$CH_CONTAINER" bash -lc "md5sum \"$1\" | awk '{print \$1}'"; }
host_size(){ stat -c%s "$1"; }
cont_size(){ docker exec -i "$CH_CONTAINER" bash -lc "stat -c%s \"$1\""; }
...
docker exec -i "$CH_CONTAINER" bash -lc "mkdir -p ${IN_CONTAINER_DIR} && (command -v md5sum >/dev/null || (apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y coreutils >/dev/null 2>&1 || true))" >/dev/null 2>&1 || true
