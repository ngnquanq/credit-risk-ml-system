...
def create_or_update_scoring_sequence(isvc_name: str, namespace: str) -> bool:
    """Patch the Knative Sequence to route events to the latest InferenceService.

    The KafkaSource (deployed via Makefile) always points to the Sequence.
    This function only patches the Sequence step to reference the latest ISVC,
    keeping channelTemplate and reply (KafkaSink) untouched.
    """
    try:
        k8s_config.load_incluster_config()
        custom_api = k8s.CustomObjectsApi()

        sequence_name = "scoring-pipeline"
        ...
        try:
            ...
            log.info(f"Patched Sequence {sequence_name} step -> {isvc_name}")
        except k8s.rest.ApiException as e:
            if e.status == 404:
                log.warning(f"Sequence {sequence_name} not found - will retry next cycle")
                return False
            raise
        ...
        log.error(f"Failed to patch Sequence: {e}")
        return False
...
def reconcile() -> None:
    ...
    # Update Sequence to route events to latest InferenceService
    if latest_version:
        service_name = f"credit-risk-{latest_version}"
        create_or_update_scoring_sequence(service_name, KSERVE_NAMESPACE)
