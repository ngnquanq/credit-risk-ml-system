...
POOL_SIZE = int(os.getenv("CH_DWH_POOL_SIZE", "20"))
...
def _init_pool():
    ...
    for _ in range(POOL_SIZE):
        ...
        _pool.put(client)
    logger.info(
        f"CH DWH pool initialized: size={POOL_SIZE} "
        f"host={settings.clickhouse_host}:{settings.clickhouse_port} db={settings.clickhouse_db_dwh}"
    )
