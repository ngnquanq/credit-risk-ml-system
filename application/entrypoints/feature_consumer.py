...
from infrastructure.external.dwh_client_ch import fetch_all_by_sk_id_curr, get_table_columns, MART_TABLES
...
class DWHFeaturesService:
        ...
        async with self._semaphore:
            try:
                with tracer.start_as_current_span("dwh_features_process", context=parent_context) as span:
                    ...
                    return dwh_features, trace_headers
            except Exception as e:
    ...
    async def run(self):
        ...
        CONCURRENCY = int(os.getenv("CH_DWH_POOL_SIZE", "20"))
        # Set executor so asyncio.to_thread can run concurrent CH queries
        loop = asyncio.get_running_loop()
        loop.set_default_executor(ThreadPoolExecutor(max_workers=CONCURRENCY))
        self._semaphore = asyncio.Semaphore(CONCURRENCY)
        logger.info(f"Starting DWH Features Service (batch_size={BATCH_SIZE}, concurrency={CONCURRENCY})...")
            ...
            while True:
                ...
                for (sk_id, _), result in zip(tasks, results):
                    if isinstance(result, Exception) or result is None:
                        continue
                    dwh_features, trace_headers = result
                    if dwh_features:
